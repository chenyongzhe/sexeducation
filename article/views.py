...
def islogin(myrequest):
    ...
    if name:
        ...
        logistr="""<div  class="logintip" id="01"><img src="%s"  style="width:50px;height:50px;%s"></div>""" %(user.imgurl,"border-radius:100%;")
    else:
        logistr="""<script> $("#login_img").css("top","-5px");</script><button  id="00"  type="button" class="btn btn-primary logintip"  data-toggle="modal" data-target="#myModal">登录</button>"""
    return logistr
...
def homepage(request):
    ...
    dataa.append(resulta[resnum-1])
    dataa.append(resulta[resnum-2])
    dataa.append(resulta[resnum-3])
    dataa.append(resulta[resnum-4])
    for i in range(len(dataa)):

        dataa[i].content = re.sub("[A-Za-z0-9\!\%\[\]\,\。<>/\"=-_.-: ;]", "", dataa[i].content)
        dataa[i].content = dataa[i].content[0:90]
...
def parent(request):
    ...
    for i in range(int(start_index), int(end_index)):
        if i == current_page:
            temp = '<li class ="active" > <a href="/jz?&p=%s">%s</a></li>' % (i, i)
            # temp = '<a class="page isactive" href="/s/?keyword='+keyword+'&p=%s">%s</a>' % (i, i)
        else:
            # temp = '<a class="page" href="/s/?keyword='+keyword+'&p=%s">%s</a>' % (i, i)
            temp = '<li> <a href="/jz?p=%s">%s</a></li>' % (i, i)
        page_list.append(temp)
    afterpage = ""
    if current_page == total_count:
        # afterpage = '<a class="page" href="#">下一页</a>'
...
def articletp(request):
    ...
    result=None
    if tp == "1":
        result = models.Article.objects.filter(type_id__in=[1,2,3,4,5,6,7,10])
    else:
...
def qsn(request):
    ...
    return render(request, 'qsn.html',
                  {'usercount': ipuser.id, 'videolist': data, 'num': num, 'page_str': page_str, "loginstr": loginstr})
